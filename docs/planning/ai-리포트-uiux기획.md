...
- 제외 범위: 연차 선택 UI, 홈 질문 기록의 연차별 탐색
...
- 결과 유형에 따라 카드형 분석과 편지형 결과를 다르게 렌더링한다.
...
- 목록에서 리포트 종류·요청일·처리 상태의 정보 위계를 명확히 해야 한다.
...
| 리포트 상세 | 생성 결과 읽기 | 목록·완료 알림 | `/(tabs)/analysis/[id]` |
...
flowchart TD
    ...
    F -->|성공| G[리포트 홈]
    F -->|실패| D
    G --> H[서버 최신순 목록에 진행 중 상태 표시]
    H -->|완료 알림 탭| I[리포트 상세]
```

- 답변 선택에서 뒤로 가면 리포트 종류 선택 화면으로 돌아간다.
- 생성 요청 성공 후 홈으로 돌아오며, 서버 최신순 목록에서 생성 중 상태를 표시한다.
...
flowchart TD
    A[리포트 홈] -->|목록 행 탭| B{상태}
    B -->|COMPLETED 탭| C[리포트 상세]
    B -->|PENDING| D[분석 중 표시·탭 비활성]
    B -->|FAILED| E[실패 표시·탭 비활성]
    C -->|사용한 답변 탭| F[사용한 답변]
    F -->|뒤로| C
```
...
- 서버가 반환한 최신 요청순을 그대로 사용하고 클라이언트에서 상태별 재정렬하지 않는다.
- 타입별 그룹이나 필터는 이번 범위에서 만들지 않는다.
- 행에는 캐릭터, 타입명, 요청일, 처리 상태를 표시한다.
...
| 상태 | 표현 | 행동 |
| --- | --- | --- |
| 정상 | 생성 카드 + 단일 최신순 목록 | `COMPLETED` 행만 탭으로 상세 이동 |
...
- 분석 중: 해당 행에 `분석 중` 문구와 회전 인디케이터를 표시한다. 정책상 동시 생성 불가라면 생성 카드도 서버 판정에 맞춰 비활성화한다.
...
- 현재 OpenAPI에 평가 API가 없으므로 평가 UI를 노출하지 않는다.

#### 상태 4종

| 상태 | 표현 | 행동 |
| --- | --- | --- |
| 정상 | 타입별 결과 + `사용한 답변` 버튼 | 결과 읽기 또는 사용한 답변 확인 |
| 결과 없음 | `COMPLETED`인데 결과가 없으면 오류 상태로 취급 | 다시 불러오기 또는 지원 가능한 오류 안내 |
...
상태 색상은 Tamagui semantic theme token을 사용하고, 타입 표식은 기존 분석 팔레트를 재사용한다.
...
| 분석 중 | `theme.statusPending` | `theme.statusPending` |
| 완료 | `theme.statusCompleted` | `theme.statusCompleted` |
| 실패 | `theme.statusFailed` | `theme.statusFailed` |
