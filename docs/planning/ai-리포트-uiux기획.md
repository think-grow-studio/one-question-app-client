...
| 사용한 답변 | 생성 당시 질문·답변 스냅샷 확인 | 상세 헤더 `답변 N개` | `/(tabs)/analysis/[id]/sources` |
...
- 헤더 우측에 `답변 {{count}}개 ›`를 표시해 별도 화면으로 이동한다.
...
- Source API 상세 계약은 서버 스펙을 전달받은 뒤 타입과 훅에 반영한다.
...
5. 상세 헤더에 `답변 N개` 진입점 추가
6. Source API 계약 확정 후 사용한 답변 화면과 쿼리 연결
7. 기존 히스토리 라우트의 진입점·딥링크 확인 후 통합 또는 정리
8. iOS·Android, 360–420dp, 라이트·다크 모드에서 전체 플로우 검증

## 8. 오픈 이슈와 제약

1. **Source API 계약 대기**: 생성 당시 질문·답변 스냅샷 목록의 필드와 페이지네이션 여부는 서버 스펙을 전달받은 후 확정한다. UI는 최대 15개라는 현재 정책상 단일 응답을 기본 가정으로 한다.
2. **이용 정책 동기화**: `docs/planning/ai-분석-리포트-이용정책-기능기획.md`는 종류당 월 2회지만 기존 `COOLDOWN` 타입·주간 카피·mock 주석은 과거 정책이다. 구현 전 서버 availability 계약과 함께 정리한다.
3. **연차 선택 UI 보류**: 현재 연차만 표시한다. 연차 전환·연차별 빈 상태·홈과의 선택 상태 공유는 이번 범위에 포함하지 않는다.
4. **히스토리 라우트 정리**: 리포트 홈으로 목록을 합치되, 외부 딥링크가 확인되기 전 `history` 경로를 즉시 삭제하지 않는다.
